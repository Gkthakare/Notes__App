...
class NotesRVAdapter(private val context: Context, private val listener: INotesRVAdapter) :
    ...
    override fun onBindViewHolder(holder: NotesRVAdapter.NoteViewHolder, position: Int) {
        ...
        if (position % 2 == 1) {
            (holder.itemView as MaterialCardView).setCardBackgroundColor(Color.parseColor("#FEFEFE"));
            //  holder.imageView.setBackgroundColor(Color.parseColor(""));
        } else {
//            (holder.itemView as MaterialCardView).setCardBackgroundColor(Color.parseColor("RED"));
